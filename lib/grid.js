...
function nextTile() {
  ...
  while (i > 0) {
    ...
    for(var t = 0; t < 3; t++) {
      ...
      if(pullTile.position === 7){
        // console.log("PullTile Position is 7 (meaning we are on row 2)! \n ---------- \n");
        if(!pullTile.compare === true){
          console.log("HOLY SHIT IT WORKED EVERYTHING IS BEAUTIFUL AND NOTHING HURT");
        }
      };

      var nullTiles = (compareTile.data === null);

      // console.log(compareTile.data)
...
// case null
          if(nullTiles){
            if(compareTile[direction]) {
              if(compareTile[direction].data !== null) {
                // console.log(i);
                // console.log(pullTile, "this is pullTile");
                // console.log('I will now increment i');
            //debugger
            i++;
            smush();
          }
      ...
      nextTile();
        // console.log(pullTile.position, "this is pullTile when nextTile ran");
    }
    console.log(this);
    // this.allTiles.forEach(function(tile) {
      // console.log('we set a tile to compare false');
      // tile.compare = null;
    // });
  }
